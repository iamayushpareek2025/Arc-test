...
export function WalletButton() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      <div className="flex items-center gap-1 sm:gap-2 rounded-xl border border-slate-800 bg-slate-900/90 p-1 shadow-sm backdrop-blur-sm">
        {/* USDC Balance Pill (hidden on ultra-small screens < 400px to prevent header overflow) */}
        <div className="hidden min-[420px]:flex items-center gap-1.5 px-2 sm:px-2.5 py-1 text-xs font-semibold text-slate-200">
        ...
        <button
          ...
          className="flex items-center gap-1.5 sm:gap-2 rounded-lg bg-slate-800/90 px-2 sm:px-2.5 py-1 text-xs font-medium text-slate-200 hover:bg-slate-700/80 hover:text-white transition-colors"
        >
          <div className="h-2 w-2 rounded-full bg-emerald-400 shadow-sm shadow-emerald-400/50 shrink-0" />
          <span className="font-mono text-[11px] sm:text-xs">{formatAddress(address, 3)}</span>
